...
while True:
    ...
    if "Spegni" in command:
        ...
        break
    if re.search(r'\s*facendo', command):
        Query = "per il momento sto cercando di imparare ad imparare"
        Txt = ""
        Speak()
        break
    if re.search(r'\s*poi', command):
        Query = "credo che insegnerò a qualcuno ad imparare"
        Txt = ""
        Speak()
        break
    if re.search(r'\s*etic', a):
        Query = "ti do la mia definizione di etica"
        Txt = ""
        Speak()
        import etica
        break
    if re.search(r'\s*amici', a):
        Query = "ti presento qualcuno dei miei amici più grandi"
        Txt = ""
        Speak()
        import robot_presentazione
        break
       
    Txt = "mi sembra di aver capito,"    
    Speak()
    Txt = "ma non ti saprei risponedere adesso!"
    Query = ""
    Speak()
